                ...
                <div class="content-center">
                  ...
                  @if(count($test) > 0)
                    <center><table>
                      ...
                      @foreach($test as $tes)
                        <tr>
                            <td>{{ $loop->iteration }}</td><td>{{ $tes->name }}</td><td><a class="btn btn-link btn-primary" href="{{ route('tutor.test.detail') }}?id={{ $tes->id }}">Detail</a></td>
                        </tr>
                      @endforeach
                    </table></center>
                  @else
...
<script>
    ...
    @if (session('status'))
    	...
    	@if (session('status') == 'NotFound')
    	   swal("We're so sorry", "Wrong data provided. Try again later", "error");
    	@endif
  	@endif
</script>
